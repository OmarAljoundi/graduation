...
import { isRangerAccount, verifyAuthToken } from './services/dashboard';
...
const updateStatus = async (_req:Request,res:Response)=>{
    ...
    dynamicTemplateData: {
    status:status === "approved" ? "قبوله" : "رفضه",
...
const service_routes = (app:express.Application) =>{
    app.get('/services',[verifyAuthToken,isRangerAccount],index)
    app.get('/services/:id',verifyAuthToken,getServiceById)
    app.put('/services/:id',[verifyAuthToken,isRangerAccount],updateStatus)
