...
const ForgetPassword = () => {
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      const response = await axios.post(api, { nationality_id})
      Promise.all([loadingTimer(1500)]).then(()=>{
        PopupActions.showModal(<OTP phoneNumber={response.data.phone_number} nationality_id={nationality_id} password={password} />)
          setLoading(false)
      })
